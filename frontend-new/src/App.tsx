...
type Scenario = {
  ...
  target_cities: { name: string; lat: number; lon: number; population: number }[];
  challenge: string;
  success_criteria: { min_deflection_distance: number; max_time_remaining: number };
};
...
type DeflectionData = {
  new_impact_location: { latitude: number; longitude: number };
  deflection: {
    distance_shifted: number;
    success: boolean;
  };
};
...
type MLOptimization = {
  optimization: {
    optimal_delta_v: number;
    expected_deflection_distance: number;
...
function App() {
  ...
  const [deflectionData, setDeflectionData] = useState<DeflectionData | null>(null);
  const [score, setScore] = useState<number>(0);
  const [loading, setLoading] = useState<boolean>(false);
  const [error, setError] = useState<string | null>(null);
  const [show3D, setShow3D] = useState<boolean>(false);
  const [showStory, setShowStory] = useState<boolean>(false);
  const [liveAsteroids, setLiveAsteroids] = useState<LiveAsteroid[]>([]);
  const [mlPrediction, setMlPrediction] = useState<MLPrediction | null>(null);
  const [mlOptimization, setMlOptimization] = useState<MLOptimization | null>(null);

  // Deflection parameters
  const [deltaV, setDeltaV] = useState<number>(0.1);
  const [timeRemaining, setTimeRemaining] = useState<number>(30);
  const [impactLocation, setImpactLocation] = useState<{ lat: number; lng: number }>({ lat: 25.7617, lng: -80.1918 });
  ...
  const loadScenario = async (scenarioId: string) => {
    ...
    try {
      ...
      setError('Failed to load scenario: ' + (err?.message || err));
    } finally {
      setLoading(false);
    }
  };
  ...
  const simulateImpact = async () => {
    ...
    try {
      ...
      setError('Failed to simulate impact: ' + (err?.message || err));
    } finally {
      setLoading(false);
    }
  };
  ...
  const deflectAsteroid = async () => {
    ...
    try {
      ...
      setError('Failed to deflect asteroid: ' + (err?.message || err));
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleTimeLoop = (loopNumber: number) => {
    console.log(`Time loop ${loopNumber} started`);
    // Handle time loop mechanics
  };

  const handleStoryProgress = (progress: number) => {
